...
/**
 * Round medals from a list of round scores (one per player, same order).
 * Standard competition ranking: ties share a medal and the next rank is skipped
 * (two golds means no silver). Only ranks 1-3 with a score above zero get a medal.
 * Returns an array of 'gold' | 'silver' | 'bronze' | null.
 */
function assignMedals(scores) {
  const names = ['gold', 'silver', 'bronze'];
  return scores.map((score) => {
    if (score <= 0) return null;
    const rank = 1 + scores.filter((other) => other > score).length;
    return names[rank - 1] || null;
  });
}

module.exports = { validateWord, scoreRound, assignMedals, MIN_WORD_LENGTH };
